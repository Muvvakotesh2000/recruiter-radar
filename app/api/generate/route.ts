...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof ZodError) {
      return NextResponse.json(
        { error: "Validation failed: " + error.errors.map((e) => e.message).join(", "), success: false },
        { status: 400 }
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!jobId) {
      return NextResponse.json({ error: "jobId is required", success: false }, { status: 400 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { data: { recruiter_count: result.recruiterCount }, success: true, error: null },
      { status: 200 }
    );
  } catch (error) {
